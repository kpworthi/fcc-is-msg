...
$(function() {
  var currentBoard =  window.location.pathname.endsWith('/')?
                      window.location.pathname.slice(3, -1):
                      window.location.pathname.slice(3);
  var url = "/api/threads/" + currentBoard;
  $('#boardTitle').text('Welcome to ' + window.location.pathname);
  document.title = `Anony-board ${currentBoard}!`
  $.ajax({
    type: "GET",
    url: url,
    success: function(data) {
      ...
      //
      // note: current set up is for readability and testing
      // should be altered before going completely live/prod
      //
      if (data === 'Board requested is invalid.'){
        boardThreads.push(`
          <div class="placeholder">
            <p>Hang on... this board doesn't exist! Try contacting the admin to get it set up for you.</p>
          </div>`);
        $('#submitNewThread').remove();
      }
      else if (data.length === 0){
        boardThreads.push(`
          <div class="placeholder">
            <p>Wait a minute... there aren't any threads yet! Try posting one, or check a different board!</p>
          </div>`)
      }
      else{
        data.forEach(function(ele) {
          ...
          thread.push('<h5>' + ele.replycount + ' replies total (' + hiddenCount + ' hidden)- <a href="' + (winPath.endsWith('/')?winPath:winPath.concat('/')) + ele._id + '">See the full thread here</a>.</h5>');
